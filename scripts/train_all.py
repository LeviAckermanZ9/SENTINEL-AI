...
def log_result(step_name: str, metrics: dict):
    ...
    if os.path.exists(RESULTS_FILE):
        ...
        data = {"steps": [], "metadata": {"started_at": time.strftime("%Y-%m-%d %H:%M:%S")}}
...
def main():
    ...
    log_result("synthetic_data", {"train_samples": n_train, "val_samples": n_val, "features": 504})
    ...
    mlp_results = train_mlp(
        ...
        epochs=20, patience=5, save_dir="./models", device=device,
    )
    log_result("mlp_training", {
    ...
    ae_results = train_autoencoder(
        X_real, epochs=20, save_path="./models/autoencoder.pt", device=device,
    )
    ...
    gan_results = train_cgan(
        X_train, y_train, epochs=30, save_dir="./models", device=device,
    )
    log_result("gan_training", {
    ...
    vae_results = train_vae(
        X_bert, epochs=20, save_dir="./models", device=device,
    )
    ...
    log_result("bilstm_test", {"attn_sum_mean": round(attn_sum, 4), "output_shape": list(logits.shape)})
