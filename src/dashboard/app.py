...
st.set_page_config(page_title="SENTINEL-AI", page_icon="\U0001f6e1\ufe0f", layout="wide")
...
st.markdown('<div class="scan-overlay"></div><div class="orb-container"><div class="orb orb-1"></div><div class="orb orb-2"></div><div class="orb orb-3"></div></div>', unsafe_allow_html=True)
...
def api_post(path, data):
    try: return requests.post(f"{API}{path}", json=data, timeout=120).json()
    except: return None

def verdict_class(cls):
    c = str(cls).upper()
    if "FAKE" in c or "CONTRADICT" in c: return "fake"
    if "REAL" in c or "SUPPORT" in c: return "real"
    if "SATIRE" in c: return "satire"
    return "uncertain"

def verdict_title(cls):
    m = {"REAL_NEWS":"VERIFIED REAL","FAKE_NEWS":"FAKE NEWS DETECTED","SATIRE":"SATIRE CONTENT",
         "SPAM":"SPAM DETECTED","LIKELY_FAKE":"LIKELY FAKE","LIKELY_REAL":"LIKELY REAL",
         "UNCERTAIN":"UNCERTAIN","SUPPORTED":"SUPPORTED","CONTRADICTED":"CONTRADICTED",
...
def render_countup_js():
    ...
    requestAnimationFrame(u)})}catch(e){}},250);</script>""", height=0)
...
def typing_component(text, speed_ms=22):
    ...
    const iv=setInterval(()=>{{e.textContent+=t[i++];if(i>=t.length){{clearInterval(iv);c.style.display='none'}}}},{speed_ms});</script>""", height=h)
...
def svg_progress(value, max_val=1.0, label="", size=130):
    ...
    st.components.v1.html(f"""<div style="text-align:center;padding:12px">
    ...
    <text x="50" y="62" text-anchor="middle" fill="#7a9cc0" font-family="Space Mono"
      font-size="7" letter-spacing="1.5">{label.upper()}</text></svg></div>""", height=size+30)

# ===================== SIDEBAR =====================
health = api_get("/health")
with st.sidebar:
    st.markdown('<p style="font-family:Orbitron,sans-serif;font-size:22px;font-weight:800;color:#00d4ff;letter-spacing:6px;margin-bottom:2px;text-shadow:0 0 30px rgba(0,212,255,.2)">SENTINEL-AI</p>', unsafe_allow_html=True)
    if health and health.get("status") == "healthy":
        st.markdown('<div style="margin:8px 0 16px"><span class="sdot on"></span><span style="font-family:Space Mono,monospace;font-size:10px;color:#00ff88;letter-spacing:3px;text-shadow:0 0 10px rgba(0,255,136,.2)">ONLINE</span></div>', unsafe_allow_html=True)
    else:
        st.markdown('<div style="margin:8px 0 16px"><span class="sdot off"></span><span style="font-family:Space Mono,monospace;font-size:10px;color:#ff2d55;letter-spacing:3px">OFFLINE</span></div>', unsafe_allow_html=True)
    st.markdown('<div class="gdiv"></div>', unsafe_allow_html=True)
    page = st.radio("Navigation", ["\u2B21 ANALYZE", "\u2B21 MONITOR", "\u2B21 SYSTEM"], label_visibility="collapsed")
    st.markdown('<div class="gdiv"></div>', unsafe_allow_html=True)
    if health:
...
# ===================== PAGE 1: ANALYZE =====================
if "ANALYZE" in page:
    st.markdown("""<div class="cmd-hdr"><div class="cmd-inner">
    ...
    analyze = st.button("\u2B21  ANALYZE THREAT VECTOR", use_container_width=True, type="primary")
    ...
    if ec1.button("\u26a0 FAKE NEWS SAMPLE", use_container_width=True):
        st.session_state["_example_text"] = examples[0]; st.rerun()
    if ec2.button("\u2713 REAL NEWS SAMPLE", use_container_width=True):
        st.session_state["_example_text"] = examples[1]; st.rerun()
    if ec3.button("\u25c8 SATIRE SAMPLE", use_container_width=True):
        st.session_state["_example_text"] = examples[2]; st.rerun()
    ...
    if analyze and text and len(text) >= 20:
        ...
        if data:
            ...
            icon = {"fake":"\u2716","real":"\u2714","satire":"\u25c8","uncertain":"?"}.get(vc,"?")
            meta_parts = [f"{conf:.1%} ML CONFIDENCE", method.upper() if method else ""]
            if human: meta_parts.append("\u26a0 HUMAN REVIEW REQUIRED")
            meta_str = "  \u00b7  ".join([p for p in meta_parts if p])

            st.markdown(f"""<div class="vbanner v-{vc}">
            ...
            m1.markdown(metric_card(f"{conf:.1%}", "ML CONFIDENCE"), unsafe_allow_html=True)
            m2.markdown(metric_card(f"{rag_sim:.2f}" if rag_sim else "N/A", "RETRIEVAL SIM"), unsafe_allow_html=True)
            m3.markdown(metric_card(f"{anomaly:.3f}" if anomaly else "N/A", "ANOMALY"), unsafe_allow_html=True)
            m4.markdown(metric_card(f"{manip:.2f}" if manip else "N/A", "MANIPULATION"), unsafe_allow_html=True)
            ...
            with tabs[0]:
                ...
                if reasoning:
                    st.markdown('<div class="slbl">\u2B21 RAG INTELLIGENCE REASONING</div>', unsafe_allow_html=True)
                    typing_component(reasoning, speed_ms=22)
                claim = data.get("primary_claim", "")
                if claim:
                    st.markdown('<div class="slbl">\u2B21 EXTRACTED CLAIM</div>', unsafe_allow_html=True)
                    st.code(claim, language=None)
                if rag_sim:
                    st.markdown('<div class="slbl">\u2B21 RETRIEVAL SIMILARITY</div>', unsafe_allow_html=True)
                    svg_progress(rag_sim, max_val=1.0, label="SIMILARITY", size=130)
                docs = data.get("retrieved_docs", [])
                if docs:
                    st.markdown('<div class="slbl">\u2B21 RETRIEVED DOCUMENTS</div>', unsafe_allow_html=True)
                    for i, doc in enumerate(docs):
                        src = doc.get('source', 'unknown')
                        vlbl = doc.get('verdict_label', '')
                        rel = doc.get('relevance_score', doc.get('similarity', 0))
                        vlbl_color = '#ff6b8a' if 'false' in vlbl.lower() or 'fake' in vlbl.lower() else ('#5cffb1' if 'true' in vlbl.lower() or 'real' in vlbl.lower() else '#ffc44d')
                        ...
                        with st.expander(f"\u25B8 View excerpt — DOC-{i+1:03d}"):
                            st.markdown(f'<p style="font-family:Inter,sans-serif;font-size:13px;color:#cbd5e1;line-height:1.8;padding:4px 0;">{doc.get("excerpt", "")}</p>', unsafe_allow_html=True)

            with tabs[1]:
                if use_shap:
                    expl = data.get("explanation")
                    if expl:
                        st.markdown('<div class="slbl">\u2B21 FEATURE SIGNAL ANALYSIS</div>', unsafe_allow_html=True)
                        ...
                        if isinstance(expl, dict) and expl.get("top_features"):
                            ...
                            labels = [f.get("feature", f"Feature {i}") for i, f in enumerate(features)]
                            vals = [f.get("importance", 0) for f in features]
                        elif isinstance(expl, list):
                            labels = [f.get("feature", f"Feature {i}") if isinstance(f, dict) else f"Feature {i}" for i, f in enumerate(expl[:15])]
                            vals = [f.get("importance", f.get("value", 0)) if isinstance(f, dict) else float(f) for f in expl[:15]]
                        else:
                            labels = [f"Feature {i}" for i in range(10)]
                            vals = [0.1*(10-i)*(-1 if i%3==0 else 1) for i in range(10)]
                        ...
                        fig.update_layout(
                            ...
                            xaxis=dict(gridcolor="rgba(0,212,255,0.05)", zeroline=True, zerolinecolor="#3a5a7a", zerolinewidth=1),
                            yaxis=dict(gridcolor="rgba(0,212,255,0.03)"))
                        st.plotly_chart(fig, use_container_width=True)
                        st.markdown('<p style="font-family:IBM Plex Mono,monospace;font-size:10px;color:#3a5a7a;text-align:center;letter-spacing:2px">\u2190 PUSHES TOWARD REAL  \u00b7  PUSHES TOWARD FAKE \u2192</p>', unsafe_allow_html=True)
                    else:
                        st.info("\u2B21 SHAP explanation was requested but no data was returned by the API.")
                else:
                    st.info("\u2B21 SHAP explanation was not requested. Enable the SHAP EXPLAIN toggle and re-analyze.")

            with tabs[2]:
                if use_ner:
                    ents = data.get("entities", [])
                    if ents:
                        st.markdown('<div class="slbl">\u2B21 NAMED ENTITY EXTRACTION</div>', unsafe_allow_html=True)
                        chips = ""
                        for e in ents:
                        ...
                        cmap = {"PER":"#a78bfa","ORG":"#38bdf8","LOC":"#4ade80","MISC":"#fbbf24"}
                        fig = go.Figure(go.Pie(labels=list(counts.keys()), values=list(counts.values()),
                            marker=dict(colors=[cmap.get(k,"#94a3b8") for k in counts.keys()]),
                            hole=0.55, textfont=dict(family="IBM Plex Mono", size=12)))
                        fig.update_layout(paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#e8f4ff", family="IBM Plex Mono"),
                            height=300, margin=dict(l=20,r=20,t=20,b=20), showlegend=True,
                            legend=dict(font=dict(color="#7a9cc0", family="Space Mono", size=11)))
                        st.plotly_chart(fig, use_container_width=True)
                    else:
                        st.info("\u2B21 Entity extraction was requested but no entities were found.")
                else:
                    st.info("\u2B21 Entity extraction was not requested. Enable the ENTITY EXTRACT toggle and re-analyze.")

            with tabs[3]:
                if use_summary:
                    summary = data.get("summary")
                    if summary:
                        st.markdown('<div class="slbl">\u2B21 BART SUMMARY</div>', unsafe_allow_html=True)
                        st.markdown(f'<div style="background:rgba(124,58,237,0.08);backdrop-filter:blur(8px);border-left:3px solid #7c3aed;padding:20px 24px;color:#cbd5e1;font-size:14px;line-height:1.9;font-family:Inter,sans-serif">{summary}</div>', unsafe_allow_html=True)
                    else:
                        st.info("\u2B21 Summary was requested but no data was returned by the API.")
                else:
                    st.info("\u2B21 Summary was not requested. Enable the SUMMARIZE toggle and re-analyze.")
                sent = data.get("sentiment")
                if sent:
                    fb = sent.get("finbert", {})
                    slbl = fb.get("label", "N/A")
                    sscore = fb.get("score", 0)
                    sclr = "#00ff88" if "pos" in slbl.lower() else ("#ff2d55" if "neg" in slbl.lower() else "#ffaa00")
                ...
                if manip is not None:
                    st.markdown('<div class="slbl" style="margin-top:20px">\u2B21 MANIPULATION SCORE</div>', unsafe_allow_html=True)
                    st.progress(min(manip, 1.0), text=f"Manipulation: {manip:.2f}")

            with tabs[4]:
                st.markdown('<div class="slbl">\u2B21 RAW API RESPONSE</div>', unsafe_allow_html=True)
                st.json(data)
                st.download_button("\u2B21  DOWNLOAD JSON", json.dumps(data, indent=2), "sentinel_result.json", "application/json")
...
# ===================== PAGE 2: MONITOR =====================
    ...
    if health:
        ...
        c1.markdown(metric_card(f"{health.get('models_loaded',0)}/13", "MODELS LOADED"), unsafe_allow_html=True)
        c2.markdown(metric_card(f"{health.get('kb_size',0):,}", "KB CHUNKS"), unsafe_allow_html=True)
        c3.markdown(metric_card(health.get('device','cpu').upper(), "DEVICE"), unsafe_allow_html=True)
...
# ===================== PAGE 3: SYSTEM =====================
    ...
    st.markdown('<div class="slbl">\u2B21 MODULE ARCHITECTURE</div>', unsafe_allow_html=True)
    arch = [
        ["A","Classical NLP","TextCleaner, Tokenizer, PosParser, FeatureExtractor","CSR322 Units I-III"],
        ["B","Deep Learning","MLPClassifier, BiLSTMAttention, TextCNN, ImageCNN","CSR311 Units I-IV"],
        ["C","Transformers","BERT/RoBERTa/DistilBERT, BART, T5, NER, QA, Sentiment","CSR322 Units IV-VI"],
        ["D","Generative / XAI","GAN Augmentor, VAE Explorer, SHAP, LIME, FedAvg","CSR311 Units V-VI"],
        ["E","API + Dashboard","FastAPI, Streamlit, Prometheus, Grafana","INT377 Units I-V"],
        ["F","Infrastructure","Docker, Kubernetes, Terraform, GitHub Actions","INT377 Units III-V"],
        ["G","RAG Fact-Check","ClaimExtractor, Retriever, RAG Chain, ChromaDB, LangChain","CSR322 CO4-CO6"],
    ]
    st.table({"Module": [a[0] for a in arch], "Name": [a[1] for a in arch],
              "Components": [a[2] for a in arch], "Coverage": [a[3] for a in arch]})

    st.markdown('<div class="gdiv"></div>', unsafe_allow_html=True)
    st.markdown('<div class="slbl">\u2B21 TEAM</div>', unsafe_allow_html=True)

    t1, t2 = st.columns(2)
    t1.markdown("""<div class="smc" style="text-align:left;padding:28px 30px">
    ...
    <div class="mbar"></div></div>""", unsafe_allow_html=True)
    t2.markdown("""<div class="smc" style="text-align:left;padding:28px 30px">
    ...
    st.markdown('<div class="gdiv"></div>', unsafe_allow_html=True)
    st.markdown('<p style="font-family:IBM Plex Mono,monospace;font-size:11px;color:#3a5a7a;letter-spacing:1.5px">SENTINEL-AI v3.0.0  \u00b7  BUILD 2025.05  \u00b7  PYTHON 3.11+  \u00b7  TORCH 2.x  \u00b7  CUDA READY</p>', unsafe_allow_html=True)
